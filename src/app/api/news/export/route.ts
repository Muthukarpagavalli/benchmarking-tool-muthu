...
function tablePage(title: string, rows: string[][], pageLabel: string) {
  const tableX = MARGIN;
  const tableW = PAGE_WIDTH - MARGIN * 2;
  const colWidths = [70, 105, 95, 95, 335, 90];
  const headers = ["Date", "Category", "Tool", "Type", "Summary", "Impact"];
  const headerH = 24;
  const rowMinH = 28;
  const rowGap = 0;
  ...
  output += `\n${textBlock(["Legal Technology Market Intelligence"], MARGIN, PAGE_HEIGHT - 46, {
    size: 9.4,
  ...
  wrappedRows.forEach((row, rowIndex) => {
    ...
    row.forEach((lines, cellIndex) => {
      ...
      output += `\n${textBlock(lines, cellX + ROW_PADDING_X, currentTop - ROW_PADDING_Y, {
        ...
        align: cellIndex === 0 || cellIndex === 5 ? "left" : "left",
      })}`;
      cellX += width;
    });
...
export async function GET(req: NextRequest) {
  ...
  if (rows.length === 0) {
    pages.push(tablePage(title, [], `Exported ${exportedOn} | Page 1 of 1`));
  } else {
    let index = 0;
    let pageNumber = 1;
    ...
    while (index < rows.length) {
      ...
      pages.push(tablePage(title, chunk, `Exported ${exportedOn} | Page ${pageNumber} of ${totalPagesEstimate}`));
      index += take;
      pageNumber += 1;
    }
  }
