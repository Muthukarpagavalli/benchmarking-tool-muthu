...
async function main() {
  ...
  await prisma.scoringFrameworkScore.deleteMany();
  await prisma.scoringFrameworkStackItem.deleteMany();
  await prisma.scoringFrameworkGapItem.deleteMany();
  await prisma.scoringFrameworkTool.deleteMany();
  await prisma.scoringFrameworkCriterion.deleteMany();
  ...
  for (const cat of categories) {
    ...
    const framework = await prisma.scoringFramework.create({
      data: {
        categoryId: category.id,
        name: `${cat.name} Internal Benchmark`,
      },
    });

    await prisma.scoringFrameworkTool.createMany({
      data: cat.tools.map((toolName, index) => ({
    ...
    await prisma.scoringFrameworkCriterion.createMany({
      data: STANDARD_CRITERIA.map((criterion, index) => ({
        frameworkId: framework.id,
        name: criterion.name,
